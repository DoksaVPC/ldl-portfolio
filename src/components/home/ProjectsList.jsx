...
function ProjectsList() {
  ...
  useEffect(() => {
    const followCursor = () => {
      ...
      let { mouseX, mouseY, targetX, targetY, distanceX, distanceY } =
      ...
      let distance =
        Math.sqrt(distanceX * distanceX + distanceY * distanceY) * 0.01;
      if (distance > 0.2) {
        distance = 0.2;
      }
      ...
      bubbleRef.current.style.transform = `translate(${
        animationStateRef.current.targetX - remToPixels(5)
      }px, ${animationStateRef.current.targetY - remToPixels(5.5)}px)`;
      bubbleBackgroundRef.current.style.transform = `rotate(${angle}deg)
            scaleX(${1 + distance}) 
            scaleY(${1 - distance})`;
    };
      ...
      <div className="exploreBubble" ref={bubbleRef}>
        <div
          className={`exploreBubble__content ${
            bubbleVisible
              ? "exploreBubble__content--show"
              : "exploreBubble__content--hide"
          }`}>
          <div
            className="exploreBubble__background"
            ref={bubbleBackgroundRef}></div>
          <span>Explore</span>
        </div>
      </div>
